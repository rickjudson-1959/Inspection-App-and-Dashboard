...
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFhdHZja2FsbnZvamx5a2Znbm16Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjQyODM3MzksImV4cCI6MjA3OTg1OTczOX0.tLCD2adzgSQZlS4D2ePLKOXL_phGk7eNgDRdDpSGN5M'
const supabase = createClient(supabaseUrl, supabaseKey)

function App() {
  // Contract Rates (these would come from contract documents)
...
const [employeeClass, setEmployeeClass] = useState('Labourer')
...
function calculateTotalCost(labour, equipment) {
  ...
  <div style={{ display: 'flex', alignItems: 'flex-end' }}>
    <button onClick={addEquipmentEntry} type="button" style={{ padding: '8px 16px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', width: '100%' }}>Add</button>
          ...
          <button onClick={addEquipmentEntry} type="button" style={{ padding: '8px 16px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Add</button>
        </div>
  ...
  <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
    <button onClick={exportToExcel} style={{ padding: '8px 16px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>
      Export to Excel
    </button>
    ...
    <select value={filterChangeOrder} onChange={(e) => setFilterChangeOrder(e.target.value)} style={{ padding: '8px' }}>
      ...
      {filteredPhotos.map((p) => (
          <div key={p.id} style={{ border: '1px solid #ddd', padding: '10px', borderRadius: '4px' }}>
            <img src={supabaseUrl + '/storage/v1/object/public/ticket-photos/' + p.photo_url} style={{ width: '100%' }} alt="Ticket" />
