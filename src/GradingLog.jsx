      ...
      <CollapsibleSection id="rowConditions" title="🛤️ RIGHT OF WAY CONDITIONS" expanded={expandedSections.rowConditions} onToggle={toggleSection}>
        <div style={gridStyle}>
          <div>
            ...
            <BufferedInput type="text" inputMode="decimal" value={gradingData.rowWidth}
              onFocus={() => handleFieldFocus('rowWidth', gradingData.rowWidth)}
              onChange={(val) => updateField('rowWidth', val)}
          ...
          <div>
            ...
            <BufferedInput type="text" inputMode="decimal" value={gradingData.rowWidthSpec}
              onFocus={() => handleFieldFocus('rowWidthSpec', gradingData.rowWidthSpec)}
              onChange={(val) => updateField('rowWidthSpec', val)}
      ...
      >
        <div style={gridStyle}>
          ...
          <div>
            ...
            <BufferedInput type="text" inputMode="decimal" value={gradingData.topsoilPileSeparation}
              onFocus={() => handleFieldFocus('topsoilPileSeparation', gradingData.topsoilPileSeparation)}
              onChange={(val) => updateField('topsoilPileSeparation', val)}
          ...
          {gradingData.pileSeparationMaintained === 'No' && (
            <div style={{ gridColumn: 'span 2' }}>
              ...
              <BufferedInput as="textarea" value={gradingData.pileSeparationIssues}
                onFocus={() => handleFieldFocus('pileSeparationIssues', gradingData.pileSeparationIssues)}
                onChange={(val) => updateField('pileSeparationIssues', val)}
      ...
      <CollapsibleSection id="topsoilStatus" title="🌱 TOPSOIL STATUS" expanded={expandedSections.topsoilStatus} onToggle={toggleSection}>
        <div style={gridStyle}>
          ...
          <div>
            ...
            <BufferedInput type="text" inputMode="numeric" value={gradingData.topsoilDepth}
              onFocus={() => handleFieldFocus('topsoilDepth', gradingData.topsoilDepth)}
              onChange={(val) => updateField('topsoilDepth', val)}
        ...
        <div style={gridStyle}>
          ...
          {gradingData.pondingObserved === 'Yes' && (
            <div>
              ...
              <BufferedInput type="text" value={gradingData.pondingLocation}
                onFocus={() => handleFieldFocus('pondingLocation', gradingData.pondingLocation)}
                onChange={(val) => updateField('pondingLocation', val)}
          ...
          {gradingData.drainageControlsInstalled === 'Yes' && (
            <div>
              ...
              <BufferedInput type="text" value={gradingData.drainageControlsType}
                onFocus={() => handleFieldFocus('drainageControlsType', gradingData.drainageControlsType)}
                onChange={(val) => updateField('drainageControlsType', val)}
      ...
      >
        <div style={gridStyle}>
          ...
          <div style={{ gridColumn: 'span 2' }}>
            ...
            <BufferedInput type="text" value={gradingData.environmentalIssues}
              onFocus={() => handleFieldFocus('environmentalIssues', gradingData.environmentalIssues)}
              onChange={(val) => updateField('environmentalIssues', val)}
                ...
                <div key={spot.id} style={{ marginBottom: '15px', padding: '15px', backgroundColor: '#fff3cd', borderRadius: '8px', border: '1px solid #ffc107' }}>
                  ...
                  <div style={gridStyle}>
                    <div>
                      ...
                      <BufferedInput type="text" value={spot.kp} onFocus={() => handleEntryFieldFocus(spot.id, 'kp', spot.kp)} onChange={(val) => updateSoftSpot(spot.id, 'kp', val)} onBlur={(e) => handleEntryFieldBlur(spot.id, 'kp', e.target.value, 'Location KP', getSoftSpotLabel(spot, idx))} placeholder="e.g. 5+350" style={inputStyle} />
                    </div>
                    <div>
                      <label style={labelStyle}>Length (m)</label>
                      <BufferedInput type="text" inputMode="decimal" value={spot.length} onFocus={() => handleEntryFieldFocus(spot.id, 'length', spot.length)} onChange={(val) => updateSoftSpot(spot.id, 'length', val)} onBlur={(e) => handleEntryFieldBlur(spot.id, 'length', e.target.value, 'Length', getSoftSpotLabel(spot, idx))} placeholder="Approx length" style={inputStyle} />
                    </div>
                    <div>
                      <label style={labelStyle}>Width (m)</label>
                      <BufferedInput type="text" inputMode="decimal" value={spot.width} onFocus={() => handleEntryFieldFocus(spot.id, 'width', spot.width)} onChange={(val) => updateSoftSpot(spot.id, 'width', val)} onBlur={(e) => handleEntryFieldBlur(spot.id, 'width', e.target.value, 'Width', getSoftSpotLabel(spot, idx))} placeholder="Approx width" style={inputStyle} />
                    </div>
                    <div>
                    ...
                    <div style={{ gridColumn: 'span 2' }}>
                      ...
                      <BufferedInput type="text" value={spot.comments} onFocus={() => handleEntryFieldFocus(spot.id, 'comments', spot.comments)} onChange={(val) => updateSoftSpot(spot.id, 'comments', val)} onBlur={(e) => handleEntryFieldBlur(spot.id, 'comments', e.target.value, 'Comments', getSoftSpotLabel(spot, idx))} placeholder="Additional details..." style={inputStyle} />
                    </div>
                  </div>
                </div>
              ))
              ...
              gradingData.crossings.entries.map((crossing, idx) => (
                <div key={crossing.id} style={{ marginBottom: '15px', padding: '15px', backgroundColor: '#d1ecf1', borderRadius: '8px', border: '1px solid #17a2b8' }}>
                  ...
                  <div style={gridStyle}>
                    ...
                    <div>
                      <label style={labelStyle}>Location (KP)</label>
                      <BufferedInput type="text" value={crossing.kp} onFocus={() => handleEntryFieldFocus(crossing.id, 'kp', crossing.kp)} onChange={(val) => updateCrossing(crossing.id, 'kp', val)} onBlur={(e) => handleEntryFieldBlur(crossing.id, 'kp', e.target.value, 'Location KP', getCrossingLabel(crossing, idx))} placeholder="e.g. 5+500" style={inputStyle} />
                    </div>
                    <div>
