...
`)
    ...
    try {
      ...
      if (originalLabour !== newLabour) {
        await logFieldChange({
          ...
          entityType: 'billing_verification',
    ...
    try {
      const updateData = { 
        ...
        ...(newStatus === 'ready_for_billing' && {
          ...
          ready_for_billing_by: adminName
        })
      }
  ...
  // Create invoice batch from selected LEMs
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f3f4f6', fontFamily: 'Arial, sans-serif' }}>
          ...
          {selectedForBilling.length > 0 && (
            <div style={{ backgroundColor: '#059669', borderRadius: '8px', padding: '16px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'white' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button onClick={() => updateBillingStatus('ready_for_billing')} style={{ padding: '10px 20px', backgroundColor: '#16a34a', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '500' }}>
                  📋 Ready for Billing
                </button>
                <button onClick={() => setShowInvoiceModal(true)} style={{ padding: '10px 20px', backgroundColor: '#7c3aed', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '500' }}>
                  📄 Create Invoice
                </button>
      ...
      {/* VERIFICATION MODAL - No Talk Side-by-Side View */}
      {showVerifyModal && verifyingLem && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: '#f3f4f6', borderRadius: '12px', width: '95%', maxWidth: '1600px', height: '90vh', display: 'flex', flexDirection: 'column' }}>
            {/* Header */}
            <div style={{ backgroundColor: '#1e3a5f', color: 'white', padding: '16px 24px', borderRadius: '12px 12px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
              <div>
                <h2 style={{ margin: 0, fontSize: '20px' }}>🔍 Verify LEM - {verifyingLem.field_log_id}</h2>
                <p style={{ margin: '4px 0 0 0', fontSize: '13px', color: '#93c5fd' }}>Date: {verifyingLem.date} | Foreman: {verifyingLem.foreman}</p>
              </div>
              <button onClick={() => { setShowVerifyModal(false); setVerifyingLem(null); }} style={{ background: 'none', border: 'none', color: 'white', fontSize: '28px', cursor: 'pointer' }}>×</button>
            </div>

            {/* MONEY BANNER - Most prominent */}
            <div style={{ backgroundColor: '#059669', color: 'white', padding: '20px 24px', display: 'flex', justifyContent: 'space-around', alignItems: 'center', flexShrink: 0 }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '12px', opacity: 0.8, textTransform: 'uppercase' }}>Labour Cost</div>
                <div style={{ fontSize: '28px', fontWeight: 'bold', fontFamily: 'monospace' }}>${(parseFloat(verifyingLem.total_labour_cost) || 0).toLocaleString()}</div>
              </div>
              <div style={{ fontSize: '32px', fontWeight: '300' }}>+</div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '12px', opacity: 0.8, textTransform: 'uppercase' }}>Equipment Cost</div>
                <div style={{ fontSize: '28px', fontWeight: 'bold', fontFamily: 'monospace' }}>${(parseFloat(verifyingLem.total_equipment_cost) || 0).toLocaleString()}</div>
              </div>
              <div style={{ fontSize: '32px', fontWeight: '300' }}>=</div>
              <div style={{ textAlign: 'center', backgroundColor: 'rgba(255,255,255,0.2)', padding: '10px 30px', borderRadius: '8px' }}>
                <div style={{ fontSize: '12px', opacity: 0.8, textTransform: 'uppercase' }}>💰 TOTAL BILLED</div>
                <div style={{ fontSize: '36px', fontWeight: 'bold', fontFamily: 'monospace' }}>
                  ${((parseFloat(verifyingLem.total_labour_cost) || 0) + (parseFloat(verifyingLem.total_equipment_cost) || 0)).toLocaleString()}
                </div>
              </div>
            </div>

            {/* Side-by-Side Content */}
            <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', padding: '16px', overflow: 'hidden' }}>
              
              {/* LEFT PANEL - Digital Data */}
              <div style={{ backgroundColor: 'white', borderRadius: '8px', border: '2px solid #2563eb', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <div style={{ backgroundColor: '#2563eb', color: 'white', padding: '12px 16px', fontWeight: '600', fontSize: '16px' }}>📊 Digital Data (Contractor LEM)</div>
                <div style={{ flex: 1, overflow: 'auto', padding: '16px' }}>
                  {/* Summary Info */}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '20px' }}>
                    <div style={{ backgroundColor: '#f9fafb', padding: '12px', borderRadius: '6px' }}>
                      <div style={{ fontSize: '11px', color: '#6b7280', textTransform: 'uppercase' }}>Field Log ID</div>
                      <div style={{ fontSize: '16px', fontWeight: '600' }}>{verifyingLem.field_log_id}</div>
                    </div>
                    <div style={{ backgroundColor: '#f9fafb', padding: '12px', borderRadius: '6px' }}>
                      <div style={{ fontSize: '11px', color: '#6b7280', textTransform: 'uppercase' }}>Account #</div>
                      <div style={{ fontSize: '16px', fontWeight: '600' }}>{verifyingLem.account_number || '-'}</div>
                    </div>
                  </div>

                  {/* Labour Entries */}
                  <h4 style={{ margin: '0 0 10px 0', color: '#2563eb', borderBottom: '2px solid #dbeafe', paddingBottom: '8px' }}>
                    Labour Entries ({(verifyingLem.labour_entries || []).length})
                  </h4>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', marginBottom: '20px' }}>
                    <thead>
                      <tr style={{ backgroundColor: '#dbeafe' }}>
                        <th style={{ padding: '8px', textAlign: 'left', border: '1px solid #bfdbfe' }}>Name</th>
                        <th style={{ padding: '8px', textAlign: 'left', border: '1px solid #bfdbfe' }}>Classification</th>
                        <th style={{ padding: '8px', textAlign: 'center', border: '1px solid #bfdbfe' }}>RT Hrs</th>
                        <th style={{ padding: '8px', textAlign: 'center', border: '1px solid #bfdbfe' }}>OT Hrs</th>
                        <th style={{ padding: '8px', textAlign: 'right', border: '1px solid #bfdbfe' }}>Rate</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(verifyingLem.labour_entries || []).length === 0 ? (
                        <tr><td colSpan="5" style={{ padding: '12px', textAlign: 'center', color: '#9ca3af' }}>No labour entries</td></tr>
                      ) : (verifyingLem.labour_entries || []).map((entry, i) => (
                        <tr key={i}>
                          <td style={{ padding: '8px', border: '1px solid #e5e7eb', fontWeight: '500' }}>{entry.name || entry.employee_name || '-'}</td>
                          <td style={{ padding: '8px', border: '1px solid #e5e7eb' }}>{entry.type || entry.classification || '-'}</td>
                          <td style={{ padding: '8px', textAlign: 'center', border: '1px solid #e5e7eb', fontFamily: 'monospace' }}>{entry.rt_hours || entry.rt || 0}</td>
                          <td style={{ padding: '8px', textAlign: 'center', border: '1px solid #e5e7eb', fontFamily: 'monospace' }}>{entry.ot_hours || entry.ot || 0}</td>
                          <td style={{ padding: '8px', textAlign: 'right', border: '1px solid #e5e7eb', fontFamily: 'monospace' }}>${entry.rt_rate || entry.rate || '-'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  {/* Equipment Entries */}
                  <h4 style={{ margin: '0 0 10px 0', color: '#2563eb', borderBottom: '2px solid #dbeafe', paddingBottom: '8px' }}>
                    Equipment Entries ({(verifyingLem.equipment_entries || []).length})
                  </h4>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                    <thead>
                      <tr style={{ backgroundColor: '#dbeafe' }}>
                        <th style={{ padding: '8px', textAlign: 'left', border: '1px solid #bfdbfe' }}>Equipment ID</th>
                        <th style={{ padding: '8px', textAlign: 'left', border: '1px solid #bfdbfe' }}>Type</th>
                        <th style={{ padding: '8px', textAlign: 'center', border: '1px solid #bfdbfe' }}>Hours</th>
                        <th style={{ padding: '8px', textAlign: 'right', border: '1px solid #bfdbfe' }}>Rate</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(verifyingLem.equipment_entries || []).length === 0 ? (
                        <tr><td colSpan="4" style={{ padding: '12px', textAlign: 'center', color: '#9ca3af' }}>No equipment entries</td></tr>
                      ) : (verifyingLem.equipment_entries || []).map((entry, i) => (
                        <tr key={i}>
                          <td style={{ padding: '8px', border: '1px solid #e5e7eb', fontFamily: 'monospace' }}>{entry.equipment_id || entry.unit_number || '-'}</td>
                          <td style={{ padding: '8px', border: '1px solid #e5e7eb' }}>{entry.type || entry.equipment_type || '-'}</td>
                          <td style={{ padding: '8px', textAlign: 'center', border: '1px solid #e5e7eb', fontFamily: 'monospace' }}>{entry.hours || 0}</td>
                          <td style={{ padding: '8px', textAlign: 'right', border: '1px solid #e5e7eb', fontFamily: 'monospace' }}>${entry.rate || '-'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  {/* Scope of Work */}
                  {verifyingLem.scope_of_work && (
                    <div style={{ marginTop: '20px', padding: '12px', backgroundColor: '#f9fafb', borderRadius: '6px' }}>
                      <div style={{ fontSize: '11px', color: '#6b7280', textTransform: 'uppercase', marginBottom: '6px' }}>Scope of Work</div>
                      <div style={{ fontSize: '13px' }}>{verifyingLem.scope_of_work}</div>
                    </div>
                  )}
                </div>
              </div>

              {/* RIGHT PANEL - Physical Documents */}
              <div style={{ backgroundColor: 'white', borderRadius: '8px', border: '2px solid #f59e0b', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <div style={{ backgroundColor: '#f59e0b', color: 'white', padding: '12px 16px', fontWeight: '600', fontSize: '16px' }}>📷 Physical LEM / Timesheet Image</div>
                <div style={{ flex: 1, overflow: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                  {verifyingLem.timesheet_photo_url ? (
                    <>
                      <img 
                        src={verifyingLem.timesheet_photo_url}
                        alt="LEM / Timesheet"
                        style={{ maxWidth: '100%', maxHeight: 'calc(100% - 40px)', objectFit: 'contain', borderRadius: '4px', boxShadow: '0 4px 12px rgba(0,0,0,0.15)', cursor: 'pointer' }}
                        onClick={() => window.open(verifyingLem.timesheet_photo_url, '_blank')}
                      />
                      <a 
                        href={verifyingLem.timesheet_photo_url} 
                        target="_blank" 
                        rel="noreferrer"
                        style={{ marginTop: '12px', color: '#92400e', fontSize: '12px' }}
                      >
                    ...
                    <div style={{ textAlign: 'center', color: '#9ca3af' }}>
                      ...
                      <p style={{ fontSize: '16px', marginBottom: '8px' }}>No timesheet image uploaded</p>
                      <p style={{ fontSize: '12px' }}>Upload via Contractor LEMs page</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Bottom Action Bar */}
            <div style={{ backgroundColor: 'white', borderTop: '2px solid #e5e7eb', padding: '20px 24px', borderRadius: '0 0 12px 12px', flexShrink: 0 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '20px', alignItems: 'end' }}>
                {/* Verified Amounts */}
                <div>
                  <div style={{ display: 'flex', gap: '16px' }}>
                    <div>
                      <label style={{ display: 'block', marginBottom: '4px', fontWeight: '500', fontSize: '12px', color: '#6b7280' }}>Verified Labour $</label>
                      <input 
                        type="number" 
                        value={verifiedLabourCost}
                        onChange={e => setVerifiedLabourCost(e.target.value)}
                        style={{ padding: '10px 12px', border: '2px solid #d1d5db', borderRadius: '6px', width: '140px', fontFamily: 'monospace', fontSize: '16px' }}
                      />
                    </div>
                    <div>
                      <label style={{ display: 'block', marginBottom: '4px', fontWeight: '500', fontSize: '12px', color: '#6b7280' }}>Verified Equipment $</label>
                      <input 
                        type="number" 
                    ...
                    <div style={{ display: 'flex', alignItems: 'end' }}>
                      <div style={{ padding: '10px 20px', backgroundColor: '#059669', color: 'white', borderRadius: '6px', fontFamily: 'monospace', fontSize: '18px', fontWeight: 'bold' }}>
                ...
                {/* Discrepancy Note */}
                <div>
                  <label style={{ display: 'block', marginBottom: '4px', fontWeight: '500', fontSize: '12px', color: '#dc2626' }}>
                    Discrepancy Note (REQUIRED if keeping Open)
                  </label>
                  <input 
                    type="text"
                    value={discrepancyNote}
                    onChange={e => setDiscrepancyNote(e.target.value)}
                    placeholder="e.g., Missing 2 hours sideboom time, Labour rate incorrect..."
                    style={{ width: '100%', padding: '10px 12px', border: '2px solid #fca5a5', borderRadius: '6px', boxSizing: 'border-box' }}
                  />
                </div>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button 
                    ...
                    style={{ padding: '12px 20px', border: '1px solid #d1d5db', borderRadius: '6px', backgroundColor: 'white', cursor: 'pointer' }}
                  >
                    Cancel
                  </button>
                  <button 
                    onClick={keepAsOpen}
                    style={{ padding: '12px 20px', border: 'none', borderRadius: '6px', backgroundColor: '#f59e0b', color: 'white', cursor: 'pointer', fontWeight: '500' }}
                  >
                    ⚠️ Keep Open
                  </button>
                  <button 
                    onClick={verifyAsMatched}
                    style={{ padding: '12px 24px', border: 'none', borderRadius: '6px', backgroundColor: '#059669', color: 'white', cursor: 'pointer', fontWeight: '600', fontSize: '16px' }}
                  >
                    ✓ Verify as MATCHED
                  </button>
                </div>
      ...
      {showInvoiceModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: 'white', borderRadius: '12px', width: '500px', maxWidth: '90%' }}>
            <div style={{ backgroundColor: '#7c3aed', color: 'white', padding: '20px', borderRadius: '12px 12px 0 0' }}>
              <h2 style={{ margin: 0, fontSize: '20px' }}>📄 Create Invoice</h2>
            </div>
            <div style={{ padding: '24px' }}>
              <div style={{ backgroundColor: '#f3f4f6', borderRadius: '8px', padding: '16px', marginBottom: '20px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  <div>
                    <div style={{ fontSize: '12px', color: '#6b7280' }}>Selected LEMs</div>
                    <div style={{ fontSize: '24px', fontWeight: 'bold' }}>{selectedForBilling.length}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '12px', color: '#6b7280' }}>Total Amount</div>
                    <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#059669' }}>${selectedTotal.toLocaleString()}</div>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                ...
                <button onClick={createInvoiceBatch} style={{ padding: '12px 24px', border: 'none', borderRadius: '6px', backgroundColor: '#7c3aed', color: 'white', cursor: 'pointer', fontWeight: '500' }}>
                  Create Invoice
                </button>
              </div>
            </div>
