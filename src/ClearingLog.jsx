      ...
      <div style={sectionStyle}>
        ...
        {expandedSections['ROW & Boundaries'] && (
          <div style={contentStyle}>
            <div style={gridStyle}>
              <div><label style={labelStyle}>Design ROW Width (m)</label><ShieldedInput type="text" inputMode="numeric" value={clearingData.rowBoundaries?.rowWidthDesign || ''} onChange={(val) => updateField('rowBoundaries', 'rowWidthDesign', val)} placeholder="Per route sheets" style={inputStyle} /></div>
              <div><label style={labelStyle}>Actual ROW Width (m)</label><ShieldedInput type="text" inputMode="numeric" value={clearingData.rowBoundaries?.rowWidthActual || ''} onChange={(val) => updateField('rowBoundaries', 'rowWidthActual', val)} placeholder="Field measured" style={inputStyle} /></div>
        ...
        {expandedSections['Power Lines'] && (
          <div style={contentStyle}>
            <div style={gridStyle}>
              ...
              <div><label style={labelStyle}>Power Line Voltage</label><ShieldedInput type="text" value={clearingData.powerLines?.powerLineVoltage || ''} onChange={(val) => updateField('powerLines', 'powerLineVoltage', val)} placeholder="e.g., 25kV, 138kV" style={inputStyle} /></div>
            </div>
          </div>
        )}
      </div>
            ...
            {(clearingData.timberDecks || []).length === 0 ? (
              ...
              (clearingData.timberDecks || []).map((deck, idx) => (
                <div key={idx} style={{ marginBottom: '15px', padding: '15px', backgroundColor: idx % 2 === 0 ? '#f8f9fa' : '#fff', borderRadius: '6px', border: '1px solid #dee2e6' }}>
                  ...
                  <div style={gridStyle}>
                    <div><label style={labelStyle}>Deck ID</label><ShieldedInput type="text" value={deck.deckId || ''} onChange={(val) => updateTimberDeck(idx, 'deckId', val)} placeholder="e.g., D-004" style={inputStyle} /></div>
                    <div><label style={labelStyle}>Start KP</label><ShieldedInput type="text" inputMode="decimal" value={deck.startKp || ''} onChange={(val) => updateTimberDeck(idx, 'startKp', val)} style={inputStyle} /></div>
                    <div><label style={labelStyle}>End KP</label><ShieldedInput type="text" inputMode="decimal" value={deck.endKp || ''} onChange={(val) => updateTimberDeck(idx, 'endKp', val)} style={inputStyle} /></div>
                    ...
                    <div><label style={labelStyle}>Min Top Diameter (cm)</label><ShieldedInput type="text" inputMode="numeric" value={deck.minTopDiameter || ''} onChange={(val) => updateTimberDeck(idx, 'minTopDiameter', val)} style={inputStyle} /></div>
                    ...
                    <div><label style={labelStyle}>Volume Estimate (m³)</label><ShieldedInput type="text" inputMode="numeric" value={deck.volumeEstimate || ''} onChange={(val) => updateTimberDeck(idx, 'volumeEstimate', val)} style={inputStyle} /></div>
                  </div>
                </div>
              ))
            )}
        ...
        {expandedSections['Grubbing & Stripping'] && (
          <div style={contentStyle}>
            <div style={gridStyle}>
              ...
              <div><label style={labelStyle}>Max Stump Height (cm)</label><ShieldedInput type="text" inputMode="numeric" value={clearingData.grubbingStripping?.stumpHeightMax || ''} onChange={(val) => updateField('grubbingStripping', 'stumpHeightMax', val)} placeholder="Spec ≤15cm" style={inputStyle} /></div>
        ...
        {expandedSections['Temp Fencing'] && (
          <div style={contentStyle}>
            <div style={gridStyle}>
              ...
              <div><label style={labelStyle}>Fencing Length (m)</label><ShieldedInput type="text" inputMode="numeric" value={clearingData.tempFencing?.tempFencingLength || ''} onChange={(val) => updateField('tempFencing', 'tempFencingLength', val)} placeholder="Total meters" style={inputStyle} /></div>
              ...
              <div><label style={labelStyle}>Number of Gates</label><ShieldedInput type="text" inputMode="numeric" value={clearingData.tempFencing?.gatesCount || ''} onChange={(val) => updateField('tempFencing', 'gatesCount', val)} style={inputStyle} /></div>
            </div>
          </div>
        )}
      </div>
        ...
        {expandedSections['Sign-Off'] && (
          <div style={contentStyle}>
            ...
            <div>
              ...
              <ShieldedInput as="textarea" value={clearingData.signOff?.notes || ''} onChange={(val) => updateField('signOff', 'notes', val)} placeholder="Additional observations, issues, or comments..." rows={4} style={{ ...inputStyle, resize: 'vertical' }} />
            </div>
          </div>
        )}
      </div>
