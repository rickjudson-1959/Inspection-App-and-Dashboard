...
export default function AuditDashboard() {
  ...
  async function exportToCSV() {
    ...
    const headers = ['Timestamp','Report ID','Report Date','User','Role','Change Type','Entity Type','Section','Field','Old Value','New Value','KP Start','KP End','Critical','Regulatory Category']
    const rows = data.map(e => [e.changed_at ? new Date(e.changed_at).toLocaleString() : '', e.report_id||'', e.report_date||'', e.changed_by_email||e.changed_by_name||'', e.changed_by_role||'', e.change_type||'', e.entity_type||'', e.section||'', e.field_name||'', e.old_value||'', e.new_value||'', e.kp_start||'', e.kp_end||'', e.is_critical?'YES':'NO', e.regulatory_category||''])
  ...
  const getTypeStyle = t => ({ create:{bg:'#d4edda',text:'#155724',label:'+ CREATE'}, edit:{bg:'#fff3cd',text:'#856404',label:'✏️ EDIT'}, delete:{bg:'#f8d7da',text:'#721c24',label:'🗑️ DELETE'} }[t] || {bg:'#e9ecef',text:'#495057',label:t||'?'})
  const getEntityLabel = t => ({ bending:'🔧 Bending', coating:'🎨 Coating', clearing:'🌲 Clearing', stringing:'📏 Stringing', welding:'⚡ Welding', ditching:'🚜 Ditching', grading:'🛤️ Grading', hdd:'🔩 HDD', hydrotest:'💧 Hydrotest', hydrovac:'🚿 Hydrovac', piling:'🪵 Piling', tiein:'🔗 Tie-In', equipment_cleaning:'🧹 Cleaning' }[t] || t || '-')
  const getRegBadge = c => ({ integrity:{bg:'#dc3545',label:'🔴 INTEGRITY'}, environmental:{bg:'#28a745',label:'🌿 ENV'}, soil_handling:{bg:'#6c757d',label:'🌍 SOIL'}, indigenous_social:{bg:'#6f42c1',label:'👥 INDIG'}, archaeological:{bg:'#fd7e14',label:'🏛️ ARCH'}, general:{bg:'#17a2b8',label:'ℹ️ GEN'} }[c])
  ...
  return (
    <div style={{ padding:'20px', maxWidth:'1600px', margin:'0 auto', fontFamily:'system-ui, sans-serif' }}>
      ...
      <div style={{ backgroundColor:'#1e3a5f', color:'white', padding:'20px', borderRadius:'8px 8px 0 0', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div><h1 style={{ margin:0, fontSize:'24px' }}>📋 Audit Dashboard</h1><p style={{ margin:'8px 0 0', opacity:0.8, fontSize:'14px' }}>System-wide change tracking</p></div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', marginBottom:'20px' }}>
        {[{l:'Total',v:stats.total,bg:'#e3f2fd',c:'#1565c0'},{l:'Creates',v:stats.creates,bg:'#d4edda',c:'#155724'},{l:'Edits',v:stats.edits,bg:'#fff3cd',c:'#856404'},{l:'Deletes',v:stats.deletes,bg:'#f8d7da',c:'#721c24'},{l:'⚠️ Critical',v:stats.critical,bg:'#f5c6cb',c:'#721c24'}].map((s,i) =>
          <div key={i} style={{ backgroundColor:s.bg, padding:'15px', textAlign:'center', border:'1px solid #dee2e6' }}><div style={{ fontSize:'24px', fontWeight:'bold', color:s.c }}>{s.v}</div><div style={{ fontSize:'12px', color:'#666' }}>{s.l}</div></div>
        )}
      </div>
      {/* Filters */}
      <div style={{ backgroundColor:'#f8f9fa', border:'1px solid #dee2e6', borderRadius:'8px', padding:'20px', marginBottom:'20px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'15px' }}>
          ...
          <div style={{ display:'flex', gap:'10px', alignItems:'center' }}>
            <label style={{ display:'flex', alignItems:'center', gap:'6px', cursor:'pointer' }}><input type="checkbox" checked={filters.criticalOnly} onChange={e => handleFilterChange('criticalOnly', e.target.checked)} style={{ width:'16px', height:'16px' }}/><span style={{ fontSize:'13px', fontWeight:'bold', color:'#dc3545' }}>⚠️ Critical Only</span></label>
            <button onClick={clearFilters} style={{ ...btn, backgroundColor:'#6c757d', color:'white' }}>Clear</button>
            <button onClick={exportToCSV} style={{ ...btn, backgroundColor:'#28a745', color:'white' }}>📥 Export CSV</button>
          </div>
        </div>
      ...
      <div style={{ backgroundColor:'white', border:'1px solid #dee2e6', borderRadius:'8px', overflow:'hidden' }}>
        <div style={{ backgroundColor:'#e9ecef', padding:'12px 20px', borderBottom:'1px solid #dee2e6', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <span style={{ fontWeight:'bold', color:'#495057' }}>{loading ? 'Loading...' : `${totalCount} entries`}</span>
        ...
        {loading ? <div style={{ padding:'40px', textAlign:'center', color:'#666' }}>Loading...</div> : auditEntries.length === 0 ? <div style={{ padding:'40px', textAlign:'center', color:'#666' }}>No entries found</div> : (
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'13px' }}>
              <thead><tr style={{ backgroundColor:'#f8f9fa' }}>{['Time','Report','User','Type','Entity','Section','Field','Old','New','KP','Status'].map(h => <th key={h} style={{ padding:'12px 8px', textAlign:'left', borderBottom:'2px solid #dee2e6', whiteSpace:'nowrap' }}>{h}</th>)}</tr></thead>
              <tbody>
                {auditEntries.map((e, i) => {
                  const ts = getTypeStyle(e.change_type), rb = getRegBadge(e.regulatory_category)
                  return (
                    <tr key={e.id||i} onClick={() => setSelectedEntry(e)} style={{ cursor:'pointer', backgroundColor:e.is_critical?'#fff5f5':i%2?'#f8f9fa':'white', borderLeft:e.is_critical?'4px solid #dc3545':'none' }}>
                      <td style={{ padding:'8px', borderBottom:'1px solid #dee2e6', fontSize:'12px' }}><div>{e.changed_at ? new Date(e.changed_at).toLocaleDateString() : '-'}</div><div style={{ color:'#888', fontSize:'10px' }}>{e.changed_at ? new Date(e.changed_at).toLocaleTimeString() : ''}</div></td>
                      <td style={{ padding:'8px', borderBottom:'1px solid #dee2e6' }}>{e.report_id ? <a href={`/report?id=${e.report_id}`} onClick={ev => ev.stopPropagation()} style={{ color:'#007bff', fontWeight:'bold' }}>#{e.report_id}</a> : '-'}</td>
                      ...
                      <td style={{ padding:'8px', borderBottom:'1px solid #dee2e6' }}><span style={{ backgroundColor:ts.bg, color:ts.text, padding:'2px 6px', borderRadius:'4px', fontSize:'11px', fontWeight:'bold' }}>{ts.label}</span></td>
                      ...
                      <td style={{ padding:'8px', borderBottom:'1px solid #dee2e6', maxWidth:'100px', fontSize:'11px' }}>{e.old_value ? <span style={{ backgroundColor:'#f8d7da', padding:'1px 4px', borderRadius:'3px' }}>{String(e.old_value).slice(0,30)}{e.old_value?.length>30?'...':''}</span> : '-'}</td>
                      <td style={{ padding:'8px', borderBottom:'1px solid #dee2e6', maxWidth:'100px', fontSize:'11px' }}>{e.new_value ? <span style={{ backgroundColor:'#d4edda', padding:'1px 4px', borderRadius:'3px' }}>{String(e.new_value).slice(0,30)}{e.new_value?.length>30?'...':''}</span> : '-'}</td>
                      <td style={{ padding:'8px', borderBottom:'1px solid #dee2e6', fontFamily:'monospace', fontSize:'12px' }}>{e.kp_start || '-'}</td>
                      <td style={{ padding:'8px', borderBottom:'1px solid #dee2e6' }}>{e.is_critical && <span style={{ backgroundColor:'#dc3545', color:'white', padding:'1px 4px', borderRadius:'4px', fontSize:'10px' }}>⚠️</span>}{rb && <span style={{ backgroundColor:rb.bg, color:'white', padding:'1px 4px', borderRadius:'4px', fontSize:'10px', marginLeft:'2px' }}>{rb.label}</span>}</td>
                    </tr>
                  )
                })}
              </tbody>
      ...
      <div style={{ marginTop:'20px', padding:'15px', backgroundColor:'#e3f2fd', borderRadius:'8px', fontSize:'13px', color:'#1565c0' }}>
        <strong>ℹ️ Info:</strong> All changes are logged with regulatory classification. Critical fields flagged automatically. Click any row for details.
      </div>
      {/* Modal */}
      {selectedEntry && (
