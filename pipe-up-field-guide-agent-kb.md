...
- **Chainage Overlap Detection**: The app checks your KP range against other blocks in the same report (same activity type) and historical reports from previous days within your organization. If an overlap is detected, a warning appears. You must provide a reason (e.g., rework, correction) before saving. The full overlap reason text prints in the PDF report.
...
5. **Concealed work photos** — Backfill is a concealed work activity. A yellow banner in the Work Photos section reminds you to take photos BEFORE backfill covers the pipe. The Health Score flags any concealed-work block without photos. This applies to: Lower-in, Backfill, Coating, HD Bores, HDD, and Frost Packing.
...
| **Clearing** | ClearingLog | **ROW & Boundaries**: design/actual width, compliant, alignment verified, boundaries flagged, TWS staked, legal survey pins protected. **Pre-Clearing Approvals**: CGR plan approved/compliance, off-ROW approvals, construction line list reviewed, landowner restrictions, land agent contact. **Environmental**: environmental inspector liaison, timing constraints, wildlife regulations, rare plant protection, ASRD commitments, ground disturbance. **Buried Facilities**: identified, locates complete, hand exposing, foreign crossings marked. **Power Lines**: present, identified, marked, clearance, voltage. **Timber Salvage**: required, compliant, merchantable salvaged, disposal method. **Timber Decks** (table): deck ID, start/end KP, owner status, species/sort, condition, cut spec, min top diameter, destination, volume estimate. **Grubbing & Stripping**: grubbing complete, stump height compliant/max, topsoil stripped, topsoil separation. **Watercourse**: present, access compliant, equipment crossing installed/type, regulatory approval, erosion controls. **Temp Fencing**: required, installed, type, length, gates installed/count. **Sign-Off**: NCR required, notes. |
| **HDD** | HDDLog | Basic info: drill contractor, mainline contractor, drill location KP, drill length, pipe size. **Bore Information**: bore ID, crossing type, entry/exit KP, bore length, subcontractor, rig number, rig size. **Pilot Hole**: mud weight (ppg), viscosity (sec/qt), pH, fluid loss (mL/30min), pitch angle, azimuth, depth, variance. **Reaming Passes** (table): pass number, reamer size (in), pullback force (lbs), drilling fluid pressure (psi), flow rate (gpm), pass complete. **Pipe Installation**: pullback force, swivel connection verified, flotation risk (Low/Medium/High), pullback speed, installation complete. **Post-Installation**: annular space verified, grout required, grout volume (m³), grout pressure (psi). **Drilling Progress**: pilot/ream1/ream2/swab/pipe pull metres and %. **Waste Management sub-log** (DrillingWasteManagement): volumes (total mixed/in storage/hauled m³), storage type/capacity, disposal facility/location/manifest/date/method, additives table (product/qty/unit/batch/supplier), vac truck (hours/equipment ID/operator), compliance testing (salinity/toxicity/metals pass/fail, lab name, test date), comments. **Steering Log sub-log** (HDDSteeringLog): guidance setup (type/model/freq/calibration), design parameters (entry/exit angle, max depth, bore length, pipe OD, min bend radius), actual angles, tolerance/completion status, station readings table (station/joint/depth/pitch/azimuth/TVD/offsets/bend radius with alerts), comments. |
| **HD Bores** | ConventionalBoreLog | Basic info: bore method (track bore/sling cradle/auger machine/directional drill), contractor, crossing type, bore length, casing size. **Bore Details**: bore ID, crossing description, design entry/exit KP, actual exit KP, subcontractor, machine ID. **Pipe Specifications**: casing OD/wall thickness, carrier OD/wall thickness (inches). **Method-Specific**: winch tension (sling cradle), boom positioning verified (sling cradle), backstop deadman confirmed (track bore). **Alignment & Grade**: start/exit pitch %, steering head used/type. **Fluid & Mud Loop**: lubrication required, total water used (m³), mud type, mud volume (m³). **Grouting**: calculated annulus volume, actual grout pumped (m³), grout variance % (alert >15%). **Asset Link**: weld ID, exit pit photo KP. Progress: today/to-date metres, % complete. **Waste Management sub-log** (DrillingWasteManagement): same structure as HDD waste management — volumes, storage, disposal, additives table, vac truck, compliance testing, comments. |
...
| **Welder Testing** | WelderTestingLog | Header: report date, test location, start/stop time. Per-welder table: welder name, project ID, ABSA #, test date, test material, wall thickness/diameter, weld procedure, pass/fail (color-coded), repair test date, repairs pass/fail. Summary shows total pass/fail counts. |
...
| **Hydrostatic Testing** | HydrotestLog | **9-section form**: (1) Test Identification — testSection, testMedium (Water/Air/Nitrogen), testType (Strength/Leak/Combined). (2) Pressure Parameters — designPressure, testPressure, startPressure, finalPressure (all kPa), pressureDropPSI (auto-calculated: (start−final)×0.145038), maxAllowableDrop (PSI). (3) Duration & Timeline — holdTime (hrs), fillStartTime, fillEndTime, testStartTime, testEndTime. (4) Water Management — waterSource, waterVolume (m³), waterDischargeLocation, waterDischargePermit, waterTemperature (°C). (5) Instrumentation — gaugeId, recorderType (Chart Recorder/Digital Logger/Manual), calibrationDate, calibrationCertificate. (6) Pressure Readings — repeatable table: time, pressure (kPa), temperature (°C), notes. (7) Personnel — testEngineer, testWitness, witnessCompany. (8) Test Result — testResult (Pass/Fail/Pending) with color-coded banner; conditional on Fail: failureReason, leaksFound (Yes/No), leakLocation, leakRepairMethod. (9) Sign-Off — ncrRequired (Yes/No/N/A), comments. PDF shows all sections with pressure readings table and color-coded result banner. |
| **Ditch** | DitchInspection | Trench specs: width, depth, cover required/actual. Pay Items (UPIs): padding/bedding KP range, metres, verified, material. BOT Checklist: rocks, debris, silt fences, wildlife ramps/gaps, grade — each Clear/OK/ISSUE. **Water Management**: pumping activity/equipment/hours, filter bags (count), discharge location, discharge permit number, notes. Soil: conditions, groundwater encountered/depth, dewatering required. Depth Compliance: minimum depth met (Y/N), with reason/signoff/role/date if not met. Comments. |
| **Grading** | GradingLog | ROW width (actual + spec), condition, access maintained. Pile separation: maintained, topsoil separation distance (m), topsoil/subsoil pile location, pile separation issues. Drainage: condition, crown maintained, ponding observed/location, drainage controls installed/type. **Environmental Controls**: silt fence installed/condition, straw bales, erosion blankets, sediment traps, environmental issues. Topsoil: stripped, depth (cm), stockpiled. **Soft Spots** (toggleable): KP, length, width, cause, action taken, resolved, comments. **Crossings** (toggleable): crossing type, KP, access maintained, signage in place, traffic control, condition, comments. |
| **Tie-in Backfill** | TieInCompletionLog | **Location**: tie-in location, from KP, to KP. **Backfill Details**: method, lift thickness, number of lifts, compaction method, compaction test required/passed, padding material/depth. CP leads, as-built measurements (renamed from "Tie-in Completion"; data key remains `tieInCompletionData`). **Third Party Crossings**: crossing type, utility type, facility type, owner, our pipe depth, third party depth, separation distance, minimum required, clearance, protection method, compliant (Y/N), surveyed by, comments. **Anodes table**: type, manufacturer, material, size, depth, weight, quantity, serial number, location, test lead installed, comments. |
...
| Chainage Overlap Detection | Warns if KP range overlaps historical reports within your organization. Reason required before saving. Full reason text prints in PDF. | Activity block — automatic |
...
A: The app detected that your KP range overlaps with a previous report for the same activity type within your organization. If this is intentional (rework, correction, different crew returning to the same area), provide a reason in the overlap explanation field — you must enter a reason before saving. The full reason text will appear in the PDF report. If it's a mistake, adjust your Start/End KP. Note: different activity types can use the same KP range without triggering an overlap (e.g., Access and Frost Packing can both cover KP 1+000 to 4+500).
...
A: Record what you actually observed — your independent labour count, equipment, and hours are the ground truth. If the ticket the contractor hands you at the end of the day shows different numbers than what you witnessed, note the discrepancy in your work description. Take a clear photo of the ticket regardless. During reconciliation, the admin will see your original ticket photo (Panel 3) next to the contractor's copy from their LEM bundle (Panel 2). If the two don't match, the admin can flag it as "Dispute-Ticket Altered."
